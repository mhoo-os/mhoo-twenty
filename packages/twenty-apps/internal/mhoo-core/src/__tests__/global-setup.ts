...
function validateDisposableLocalApiUrl(apiUrl: string) {
  const url = new URL(apiUrl);
  const loopbackHostnames = new Set(['127.0.0.1', 'localhost', '::1', '[::1]']);

  if (url.protocol !== 'http:' || !loopbackHostnames.has(url.hostname)) {
    throw new Error(
      'Integration tests may only target an explicit loopback HTTP Twenty server.',
    );
  }
}
...
function validateEnv(): { apiUrl: string; apiKey: string } {
  ...
  validateDisposableLocalApiUrl(apiUrl);
