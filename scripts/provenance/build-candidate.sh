...
[[ "$SOURCE_REVISION" =~ ^[0-9a-f]{40}$ ]] || fail "SOURCE_REVISION must be a full commit SHA"
git cat-file -e "${SOURCE_REVISION}^{commit}" || fail "source revision is not available locally"
