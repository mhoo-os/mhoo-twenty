...
| `ci-front.yaml` / `classify-front-change`, `front-sb-build`, `front-sb-test`, `front-sb-screenshots`, `front-task`, `front-build`, `ci-front-fast-status-check`, `ci-front-status-check` | Runs exact-head frontend classification, lint/typecheck/tests/build, deterministic build-artifact custody, and policy-selected Storybook validation. | ADAPT | Require full Front and Storybook validation for ordinary/control PRs and all main/merge-queue runs; allow only a newly registered, exact-blob official Twenty frontend backport to use the fast PR gate. | The inherited larger-runner label requires an unavailable paid plan. The fail-closed registry and classifier replace package-wide all-or-nothing selection without trusting labels or prose, while main still produces complete Storybook and screenshot evidence. |
...
| `publish-twenty-v2.30.1-candidate-6.yml` / `publish` | Candidate 6-only manual digest-only custody control. | KEEP, BLOCKED PENDING RE-REVIEW | It verifies the immutable annotated Git source tag separately from the OCI identity, builds an OCI layout once, publishes only its canonical digest through authenticated OCI API calls, reads back the registry digest/body, then signs and records evidence before the final receipt. | There is no Candidate 6 OCI tag and no claim that GitHub Actions concurrency makes a mutable registry tag safe. The remediation did not dispatch or publish it; Candidate 5 is not inspected or modified. |
