...
| `publish-twenty-v2.30.1-candidate-6.yml` / `publish` | Candidate 6-only manual successor custody control. | KEEP | Dispatch only from reviewed `main`; it hard-codes and verifies the annotated tag, peeled commit/tree, registry destination, and non-replacement rule before publication. | It checks out Candidate 6's frozen tagged source rather than the later workflow-control commit; Candidate 5 is not inspected or modified. |
