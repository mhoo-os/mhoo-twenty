...
| `ci-front.yaml` / `changed-files-check`, `front-sb-build`, `front-sb-test`, `front-sb-screenshots`, `front-task`, `front-build`, `ci-front-status-check` | Runs frontend lint/typecheck/tests/build and produces screenshot artifacts. | KEEP | Preserve screenshot artifact generation. | Visual artifacts remain useful without the downstream TwentyHQ Argos dispatcher. |
