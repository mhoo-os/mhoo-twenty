...
description: >-
  Audit pull-request trajectories against a repository's own documented AI layer:
  rules, skills, and workflow definitions. Mine real PR history, build or refresh
  the compliance rubric, install the live-review workflow, or review a specific PR
  using deterministic evidence followed by rubric-based judgment and human review.
  Use for requests to audit PR history, check whether an agent follows the repo's
  workflow, build a trajectory rubric, review PR process compliance, or set up
  automatic trajectory review. This is not a general code review, security scan,
  or judgment of whether a fix is good; it evaluates whether the documented
  process and required self-validation were followed.
---
...
skill installed at `.agents/skills/pr-trajectory-audit/` inside it (the normal
...
2. **Read the repo's own AI layer FIRST, before mining anything.** Find and read
   ...
   entirely), `.claude/commands/`/`.agents/skills/*/SKILL.md`, `.cursorrules`, or
...
3. **Mine the PR history:**
   ```
   uv run .agents/skills/pr-trajectory-audit/scripts/mine_prs.py mine <owner/repo> --limit <N, default 1000> --top 10
   ```
...
4. **For each candidate, check it against step 2's list, not against generic
   ...
   .agents/skills/pr-trajectory-audit/scripts/mine_prs.py scope -`), read `gh pr
...
1. **Run Tier 1 evidence-gathering:**
   ```
   uv run .agents/skills/pr-trajectory-audit/scripts/mine_prs.py checks <owner/repo> <pr_number> --json
   ```
